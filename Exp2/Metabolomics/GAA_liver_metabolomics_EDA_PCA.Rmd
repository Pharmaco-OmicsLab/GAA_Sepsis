...
subtitle: "Liver metabolomics - EDA - PCA"
author: 
...
group_order = Get_group_level(
  dir = "../2MetaboAnalyst",
  filename = "2Merged_annotated_POS_NEG_Exp2.csv"
)
...
### Read metadata
metadata = read.csv(
  paste(dir_pca, "0GAA_metadata_Exp2.csv", sep = "/")
) %>% mutate(Gender = toupper(Gender))

# Visualization
Exp2 <- as_tibble(pca$x[,1:2]) %>%
                 mutate(Sample = names(input_pca)) %>% 
                 inner_join(metadata,
                            by = "Sample") %>% 
                 mutate(Group = factor(Group, levels = c("GLUCON", "GAACON", "GLUSE", "GAASE"),labels = c("CON-sGLU","CON-rGLU-GAAs","SE-sGLU","SE-rGLU-GAAs"))) %>% 
...
ggsave(Exp2,filename = "../2MetaboAnalyst/PCAplot_Exp2.png",width = 7.5,height = 5,device = "png")
```
