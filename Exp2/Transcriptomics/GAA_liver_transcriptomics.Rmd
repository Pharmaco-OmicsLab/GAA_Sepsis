...
PCAplot <- ggscatter(pcaData,x = "PC1", y = "PC2", color = "Group", size = 1.5, star.plot = TRUE, star.plot.lwd = 0.2)+
  scale_color_manual(values = c("#81DDA1","#FFB9AE","#009193", "#EB6F69"))+
  stat_ellipse(aes(x=PC1,y=PC2,fill=Group),geom="polygon",level=0.95, alpha=0.15)+
  scale_fill_manual(values = c("#81DDA1","#FFB9AE","#009193", "#EB6F69"))+
...
ggsave(Volplot,filename =paste( "../3.Processed figures/2.Volplot_",Comparison,".png"),width = 10,height = 5,device = "png")
```
...
write.xlsx(DESeq2_GSEA_group3_group4,  file = paste("../2.Processed data/5.DESeq2_GSEA_", group3, "_",group4,".xlsx", sep = ""),  rowNames = FALSE,colNames = TRUE, borders = "columns")

DESeq2_GSEA_group1_group2 <- read.xlsx("../2.Processed data/5.DESeq2_GSEA_GAACON_GLUCON.xlsx")
DESeq2_GSEA_group3_group4 <- read.xlsx("../2.Processed data/5.DESeq2_GSEA_GAACON_GLUCON.xlsx")
```
...
write.xlsx(GSEA_KEGG_DEseq2_group3_group4, file = paste("../2.Processed data/6.GSEA_KEGG_DEseq2_",group3,"_",group4,".xlsx", sep =""), rowNames = FALSE,colNames = TRUE, borders = "columns")
...
set.seed(123458)
#SE
GSEA_GOBP_DEseq2_group1_group2 <-  gseGO(geneList      = geneList_DESeq2_group1_group2,
                                         OrgDb         = org.Ss.eg.db,
                                         ont           = "BP",
                                         minGSSize     = 10,
                                         pvalueCutoff  = 0.05,
...
#CON
GSEA_GOBP_DEseq2_group3_group4 <-  gseGO(geneList      = geneList_DESeq2_group3_group4,
                                         OrgDb         = org.Ss.eg.db,
                                         ont           = "BP",
                                         minGSSize     = 10,
                                         pvalueCutoff  = 0.05,
...
- Dot plot
```{r warning=FALSE,message=FALSE, scale = 1.5, dpi=300}
dotplot(GSEA_GOBP_DEseq2_group1_group2, showCategory = 20, split=".sign") +
  facet_grid(.~.sign) +
  #theme_Publication(base_size = 8) +
  scale_y_discrete(labels = scales::label_wrap(100)) +
  scale_size(range = c(1, 4)) +
  theme(axis.text.x = element_text(size = 8),
        axis.text.y = element_text(size = 8))
dotplot(GSEA_GOBP_DEseq2_group3_group4, showCategory = 20, split=".sign") +
  facet_grid(.~.sign) +
  #theme_Publication(base_size = 8) +
  scale_y_discrete(labels = scales::label_wrap(100)) +
  scale_size(range = c(1, 4)) +
...
```{r}
#SE
GSEA_GOBP_DEseq2_group1_group2 <- as.data.frame(GSEA_GOBP_DEseq2_group1_group2)
colnames(GSEA_GOBP_DEseq2_group1_group2)
GSEA_GOBP_DEseq2_group1_group2 <- GSEA_GOBP_DEseq2_group1_group2 %>%
  mutate(Count = sapply(strsplit(core_enrichment, "/"), length))
GSEA_GOBP_DEseq2_group1_group2$GeneRatio <- (GSEA_GOBP_DEseq2_group1_group2$Count) / (GSEA_GOBP_DEseq2_group1_group2$setSize)
GSEA_GOBP_DEseq2_group1_group2$Sign <- ifelse(GSEA_GOBP_DEseq2_group1_group2$NES < 0, "Suppressed", "Activated")
GSEA_GOBP_DEseq2_group1_group2 <- GSEA_GOBP_DEseq2_group1_group2[order(GSEA_GOBP_DEseq2_group1_group2$Sign != "Activated", -GSEA_GOBP_DEseq2_group1_group2$NES), ]
#CON
GSEA_GOBP_DEseq2_group3_group4 <- as.data.frame(GSEA_GOBP_DEseq2_group3_group4)
colnames(GSEA_GOBP_DEseq2_group3_group4)
GSEA_GOBP_DEseq2_group3_group4 <- GSEA_GOBP_DEseq2_group3_group4 %>%
  mutate(Count = sapply(strsplit(core_enrichment, "/"), length))
GSEA_GOBP_DEseq2_group3_group4$GeneRatio <- (GSEA_GOBP_DEseq2_group3_group4$Count) / (GSEA_GOBP_DEseq2_group3_group4$setSize)
GSEA_GOBP_DEseq2_group3_group4$Sign <- ifelse(GSEA_GOBP_DEseq2_group3_group4$NES < 0, "Suppressed", "Activated")
GSEA_GOBP_DEseq2_group3_group4 <- GSEA_GOBP_DEseq2_group3_group4[order(GSEA_GOBP_DEseq2_group3_group4$Sign != "Activated", -GSEA_GOBP_DEseq2_group3_group4$NES), ]

```

- Export results
```{r}
write.xlsx(GSEA_GOBP_DEseq2_group1_group2,  file = paste("../2.Processed data/6.GSEA_GOBP_DEseq2_",group1,"_",group2,".xlsx", sep = ""),  rowNames = FALSE,colNames = TRUE, borders = "columns")
write.xlsx(GSEA_GOBP_DEseq2_group3_group4, file = paste("../2.Processed data/6.GSEA_GOBP_DEseq2_",group3,"_",group4,".xlsx", sep =""), rowNames = FALSE,colNames = TRUE, borders = "columns")
...
```{r}
library(ReactomePA)
GSEA_Reactome_DEseq2_group1_group2 <- gsePathway(geneList      = geneList_DESeq2_group1_group2,
                                                 organism      = "pig",  #http://reactome.org/content/schema/instance/browser/49633
                                                 minGSSize     = 10,
                                                 pvalueCutoff  = 0.05,
                                                 pAdjustMethod = "fdr",
                                                 eps           = 0,
                                                 verbose       = FALSE)
GSEA_Reactome_DEseq2_group3_group4 <- gsePathway(geneList      = geneList_DESeq2_group3_group4,
                                                 organism      = "pig",  #http://reactome.org/content/schema/instance/browser/49633
                                                 minGSSize     = 10,
                                                 pvalueCutoff  = 0.05,
                                                 pAdjustMethod = "fdr",
...
- Dot plot
```{r warning=FALSE,message=FALSE, scale = 1.5, dpi=300}
dotplot(GSEA_Reactome_DEseq2_group1_group2, showCategory = 20, split=".sign") +
  facet_grid(.~.sign) +
  #theme_Publication(base_size = 8) +
  scale_y_discrete(labels = scales::label_wrap(100)) +
  scale_size(range = c(1, 4)) +
  theme(axis.text.x = element_text(size = 8),
        axis.text.y = element_text(size = 8))
dotplot(GSEA_Reactome_DEseq2_group3_group4, showCategory = 20, split=".sign") +
  facet_grid(.~.sign) +
  #theme_Publication(base_size = 8) +
  scale_y_discrete(labels = scales::label_wrap(100)) +
  scale_size(range = c(1, 4)) +
...
```{r}
#SE
GSEA_Reactome_DEseq2_group1_group2 <- as.data.frame(GSEA_Reactome_DEseq2_group1_group2)
colnames(GSEA_Reactome_DEseq2_group1_group2)
GSEA_Reactome_DEseq2_group1_group2 <- GSEA_Reactome_DEseq2_group1_group2 %>%
  mutate(Count = sapply(strsplit(core_enrichment, "/"), length))
GSEA_Reactome_DEseq2_group1_group2$GeneRatio <- (GSEA_Reactome_DEseq2_group1_group2$Count) / (GSEA_Reactome_DEseq2_group1_group2$setSize)
GSEA_Reactome_DEseq2_group1_group2$Sign <- ifelse(GSEA_Reactome_DEseq2_group1_group2$NES < 0, "Suppressed", "Activated")
GSEA_Reactome_DEseq2_group1_group2 <- GSEA_Reactome_DEseq2_group1_group2[order(GSEA_Reactome_DEseq2_group1_group2$Sign != "Activated", -GSEA_Reactome_DEseq2_group1_group2$NES), ]
#CON
GSEA_Reactome_DEseq2_group3_group4 <- as.data.frame(GSEA_Reactome_DEseq2_group3_group4)
colnames(GSEA_Reactome_DEseq2_group3_group4)
GSEA_Reactome_DEseq2_group3_group4 <- GSEA_Reactome_DEseq2_group3_group4 %>%
  mutate(Count = sapply(strsplit(core_enrichment, "/"), length))
GSEA_Reactome_DEseq2_group3_group4$GeneRatio <- (GSEA_Reactome_DEseq2_group3_group4$Count) / (GSEA_Reactome_DEseq2_group3_group4$setSize)
GSEA_Reactome_DEseq2_group3_group4$Sign <- ifelse(GSEA_Reactome_DEseq2_group3_group4$NES < 0, "Suppressed", "Activated")
GSEA_Reactome_DEseq2_group3_group4 <- GSEA_Reactome_DEseq2_group3_group4[order(GSEA_Reactome_DEseq2_group3_group4$Sign != "Activated", -GSEA_Reactome_DEseq2_group3_group4$NES), ]

```

- Export results
```{r}
write.xlsx(GSEA_Reactome_DEseq2_group1_group2,  file = paste("../2.Processed data/6.GSEA_Reactome_DEseq2_",group1,"_",group2,".xlsx", sep = ""),  rowNames = FALSE,colNames = TRUE, borders = "columns")
write.xlsx(GSEA_Reactome_DEseq2_group3_group4, file = paste("../2.Processed data/6.GSEA_Reactome_DEseq2_",group3,"_",group4,".xlsx", sep =""), rowNames = FALSE,colNames = TRUE, borders = "columns")
...
#CON
group3_group4 <- read.xlsx(paste("../2.Processed data/6.GSEA_KEGG_DEseq2_",group3,"_",group4,".xlsx", sep =""))
...
group1_group2_dotplot <- ggplot(group1_group2_KEGG_filter, aes(x = `Gene ratio`, y= Term))+
  ...
  scale_color_gradientn(breaks = c(3e-02, 1e-6, 1e-10), colours=c("#f7ca64", "#46bac2", "#7e62a3"), trans = "log10", guide=guide_colorbar(reverse=T, order=1)) +
...
ggsave(group1_group2_dotplot,filename = paste("../3.Processed figures/3.",group1,"_",group2,"_dotplot.png", sep = ""),width = 10,height = 16,device = "png")
```
...
group3_group4_dotplot <- ggplot(group3_group4_KEGG_filter, aes(x = `Gene ratio`, y= Term))+
  ...
  geom_segment(aes(xend=0, yend = Term)) +
  #scale_x_continuous(breaks = seq(0.0, 0.6, by = 0.1)) +
  scale_color_gradientn(breaks = c(4e-02, 1e-3, 1e-5), colours=c("#f7ca64", "#46bac2", "#7e62a3"), trans = "log10", guide=guide_colorbar(reverse=T, order=1)) +
  theme_dose(font.size = 12) +
  scale_size_continuous(range=c(3, 7)) +
  facet_grid(vars(KEGG_PATHWAY_CLASS2), vars(Sign), scale = 'free', space = 'free_y', switch='y')+
  theme(strip.text.y.left = element_text(angle = 0, size = 12))+
...
ggsave(group3_group4_dotplot,filename = paste("../3.Processed figures/3.",group3,"_",group4,"_dotplot.png", sep =""),width = 9,height = 9,device = "png")
```