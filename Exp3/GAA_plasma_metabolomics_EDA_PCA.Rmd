...
### Read metadata
metadata = read.csv(
  paste(dir_pca, "0GAA_metadata_Exp3SE.csv", sep = "/")
) %>% mutate(Gender = toupper(Gender))

# Visualization
Exp3 <- as_tibble(pca$x[,1:2]) %>%
                 mutate(Sample = names(input_pca)) %>% 
                 inner_join(metadata,
                            by = "Sample") %>% 
                 mutate(Group = factor(Group, levels = c("rGLUSE", "GAASE"),labels = c("SE-rGLU","SE-rGLU-GAAs"))) %>% 
...
ggsave(Exp3,filename = "../2MetaboAnalyst/PCAplot_Exp3.png",width = 7.5,height = 5,device = "png")
```
